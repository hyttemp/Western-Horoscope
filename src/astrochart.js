...
(function( astrology ) {
	...
	astrology.ASPECTS = { 
		...
		"sextile":     {"degree": 60,  "orbit": 6,  "color": "#4169E1"},		
...
(function( astrology ) {
    ...
    if(settings){
        ...
        // ---------------------------------------------------------------
        // ASPECTS_ORBIT: 允許外部只覆蓋特定相位的容忍度（orbit）
        // 例如：{ conjunction: 8, square: 6 }
        // 若沒有傳入，則沿用 astrology.ASPECTS 原始值
        // ---------------------------------------------------------------
        if(settings.ASPECTS_ORBIT){
            for(var aspectName in settings.ASPECTS_ORBIT){
                if(
                    settings.ASPECTS_ORBIT.hasOwnProperty(aspectName) &&
                    astrology.ASPECTS.hasOwnProperty(aspectName)
                ){
                    astrology.ASPECTS[aspectName].orbit = settings.ASPECTS_ORBIT[aspectName];
                }
            }
        }
    }
...
(function( astrology ) {
	...
	astrology.Radix.prototype.aspects = function( customAspects ){
		var aspectsList = customAspects != null && Array.isArray(customAspects) ? 
						  customAspects : 
						  new astrology.AspectCalculator( this.toPoints, { aspects: astrology.ASPECTS } ).radix( this.data.planets );
